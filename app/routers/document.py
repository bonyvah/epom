...
async def get_project_documents(
    id: UUID, pagination: PaginationParamsDep, current_user: CurrentUser, db: DBSession
):
    return await document_service.get_project_documents(
        id, pagination, current_user, db
    )


@router.post("/project/{id}/document", response_model=DocumentResponse)
async def upload_document_to_project(
    id: UUID, file: UploadFile, current_user: CurrentUser, db: DBSession
):
...
async def update_document(
    id: UUID, body: DocumentUpdate, current_user: CurrentUser, db: DBSession
):
