...
async def get_projects(
    pagination: PaginationParamsDep, current_user: CurrentUser, db: DBSession
):
    return await project_service.get_projects(pagination, current_user, db)


@router.post(
    "/project", response_model=ProjectResponse, status_code=status.HTTP_201_CREATED
)
...
async def update_project_info(
    id: UUID, body: ProjectUpdate, current_user: CurrentUser, db: DBSession
):
...
async def grant_access_to_project(
    project_id: UUID, body: InviteUserRequest, current_user: CurrentUser, db: DBSession
):
    await project_service.grant_access_to_project(
        project_id, body.user_id, current_user, db
    )
